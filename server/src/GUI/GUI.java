...
public class GUI extends JFrame {
    ...
    private void createUIComponents() {
        dtm = new DefaultTableModel(null, GUISetting.TABLE_COLS);
        dtm.setColumnIdentifiers(GUISetting.TABLE_COLS);
        ...
        racerStatTable = new JTable(dtm) {
            ...
            public Component prepareRenderer(TableCellRenderer renderer, int row, int column) { // interchange background color for each row
                ...
                if (!c.getBackground().equals(getSelectionBackground())) {
                    c .setBackground(row % 2 == 0 ? Color.WHITE : GUISetting.BORDER_COLOR);
                }
                return c;
            }
        };
    ...
    private void setSpinnerUI() {
        playersSpinner.setModel(new SpinnerNumberModel(GameSetting.INIT_NUM_OF_RACERS, GameSetting.MIN_NUM_OF_RACERS, GameSetting.MAX_NUM_OF_RACERS, 1));
        roundsSpinner.setModel(new SpinnerNumberModel(GameSetting.INIT_RACE_LENGTH, GameSetting.MIN_RACE_LENGTH, GameSetting.MAX_RACE_LENGTH, 1));
        ...
        playersSpinner.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(ChangeEvent e) {
                GameController.getInstance().setNumOfRacers((int)playersSpinner.getValue());
            }
        });

        // update race length in server
        roundsSpinner.addChangeListener(e -> GameController.getInstance().setRaceLength((int) roundsSpinner.getValue()));
    ...
    private void setButtonUI() {
        startServerButton.setBackground(GUISetting.LIGHT_ORANGE);
        startServerButton.setBorder(new LineBorder(GUISetting.LIGHT_ORANGE));
        ...
        startServerButton.addActionListener(e -> {
            ...
            NetworkController.getInstance().openServerSocket(); // open server socket and connect to database

            disableComponentAfterOpenConnection(); // disable changeability of configuration
            setTableUI();
        });

        sendQuestionButton.setBackground(GUISetting.LIGHT_ORANGE);
        sendQuestionButton.setBorder(new LineBorder(GUISetting.LIGHT_ORANGE));
        ...
        ActionListener alGiveQuestion = new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                GameController.getInstance().giveQuestion();
            }
        };
    ...
    private void setSeparatorUI() {
        ...
        for (int i = 0; i < hSep.size(); ++i) {
            hSep.get(i).setBackground(GUISetting.BORDER_COLOR);
            hSep.get(i).setForeground(GUISetting.BORDER_COLOR);
            hSep.get(i).setBorder(BorderFactory.createMatteBorder(3, 0, 0, 0, GUISetting.BORDER_COLOR));
        }
        ...
        for (int i = 0; i < vSep.size(); ++i) {
            ...
            vSep.get(i).setBackground(GUISetting.BORDER_COLOR);
            vSep.get(i).setForeground(GUISetting.BORDER_COLOR);
            vSep.get(i).setBorder(BorderFactory.createMatteBorder(0, 1, 0, 2, GUISetting.BORDER_COLOR));
        }
    }
    ...
    private void setTableUI() {
        ...
        int height = (GameController.getInstance().getNumOfRacers() + 4) * GUISetting.ROW_HEIGHT;
        int width = 0;
        for (int i = 0; i < GUISetting.PREFERRED_WIDTH.length; ++i) { width += GUISetting.PREFERRED_WIDTH[i]; }
        ...
        racerStatTable.setRowHeight(GUISetting.ROW_HEIGHT);
        ...
        racerStatTable.getTableHeader().setPreferredSize(new Dimension(-1, GUISetting.ROW_HEIGHT-1));
        racerStatTable.getTableHeader().setBackground(GUISetting.LIGHT_ORANGE);
        ...
        for (int i = 0; i < GUISetting.TABLE_COLS.length; ++i) {
            racerStatTable.getColumnModel().getColumn(i).setMaxWidth(GUISetting.PREFERRED_WIDTH[i]); // set column width
    ...
    public void addSRacerToUI(String racerName, int gain, int status, int position) {
        ...
        dtm.addRow(new Object[]{racerName, "Empty", gainStr, GameSetting.STATUS_STRING[status], position});
    }
    ...
    public void updateSRacerToUI(String racerName, int gain, int status, int position) {
        ...
        for (int i = 0; i < GameController.getInstance().getNumOfRacers(); ++i) {
            String str = (String)dtm.getValueAt(i, 0);

            if (checkIfEqualToNickname(str, racerName)) {
                dtm.setValueAt(gainStr, i, 2);
                dtm.setValueAt(GameSetting.STATUS_STRING[status], i, 3);
    ...
    public void renewRacerNickname(Racer racer) {
        for (int i = 0; i < GameController.getInstance().getNumOfRacers(); ++i) {
            String str = (String)dtm.getValueAt(i, 0);
