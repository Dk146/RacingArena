...
public class GUI extends JFrame {
    ...
    public static GUI getInstance() {
        if (GUI == null) {
            GUI = new GUI(GUISetting.GAME_NAME);
        }
        return GUI;
    }

    public GUI(String _gameName) {
        super(_gameName);
        GUI = this;
    ...
    private void setChangeClientGUI() {
        ...
        racerStatusLabel.setForeground(GUISetting.LIGHT_ORANGE);
        ...
        // set buttons
        joinServerButton.setBackground(GUISetting.LIGHT_ORANGE);
        joinServerButton.setForeground(GUISetting.LIGHT_ORANGE);
        joinServerButton.setBorder(new LineBorder(GUISetting.LIGHT_ORANGE));
        joinServerButton.setFont(new Font("Arial", Font.BOLD,  16));
        joinServerButton.setEnabled(false);

        submitAnswerButton.setBackground(GUISetting.LIGHT_ORANGE);
        submitAnswerButton.setForeground(GUISetting.LIGHT_ORANGE);
        submitAnswerButton.setBorder(new LineBorder(GUISetting.LIGHT_ORANGE));
        submitAnswerButton.setFont(new Font("Arial", Font.BOLD,  16));
        submitAnswerButton.setEnabled(false);
    }

    private void setPermanentClientGUI() {
        this.getContentPane().setBackground(GUISetting.BACKGROUND_COLOR);
    ...
    private void setSeparatorUI() {
        ...
        for (int i = 0; i < sep.size(); ++i) {
            sep.get(i).setBackground(GUISetting.BORDER_COLOR);
            sep.get(i).setForeground(GUISetting.BORDER_COLOR);
            sep.get(i).setBorder(BorderFactory.createMatteBorder(3, 0, 0, 0, GUISetting.BORDER_COLOR));
        }
    }
    ...
    private void setTextBoxUI() {
        ...
        enterUsername.setBorder(GUISetting.BORDER);
        ...
        enterUsername.addKeyListener(new KeyAdapter() {
            ...
            public void keyTyped(KeyEvent e) { // if nickname exceeds MAX_NICKNAME_LENGTH then prevent racer to type more
                if (enterUsername.getText().length() >= GUISetting.MAX_NICKNAME_LENGTH) {
                    e.consume();
                }
            }
            @Override
        ...
        enterAnswer.setBorder(GUISetting.BORDER);
    ...
    private void createCountDownTimer() {
        ...
        timerBar.setBorder(new LineBorder(GUISetting.BORDER_COLOR, 2));
        timerBar.setForeground(Color.BLACK);
        timerBar.setBackground(GUISetting.BACKGROUND_COLOR);
        timerBar.setUI(new BasicProgressBarUI() {
            protected Color getSelectionBackground() { return Color.BLACK; }
            protected Color getSelectionForeground() { return GUISetting.BACKGROUND_COLOR; }
        });

        timerBar.setMaximum(GUISetting.TIMER_MAX);
        timerBar.setValue(GameSetting.MAX_TIMER_SEC);

        timerBar.setString(Integer.toString(GameSetting.MAX_TIMER_SEC));
    }
    ...
    private void setColorButtonUI() {
        ...
        for (int i = 0; i < colorButtons.size(); ++i) {
            ...
            colorButtons.get(i).setBackground(GUISetting.BACKGROUND_COLOR);
            colorButtons.get(i).setForeground(GUISetting.LIGHT_ORANGE);
    ...
    private void setButtonAction() {
        ...
        joinServerButton.addActionListener(e -> {
            ...
            GameController.getInstance().getCRacer().setNickname(userNickname);

            CSenLogin cdLogin = new CSenLogin(NetworkSetting.CMD.CMD_LOGIN, userNickname);
            Network.getInstance().send(cdLogin);
    ...
    private CompoundBorder createProgressBarBorder(int rightThickness) {
        Border line = BorderFactory.createMatteBorder(0, 0, 0, rightThickness, GUISetting.BORDER_COLOR);
    ...
    private JProgressBar createRacerStatusBar(int rightThickness) {
        ...
        tmpBar.setForeground(GUISetting.BACKGROUND_COLOR);
        tmpBar.setBackground(GUISetting.BACKGROUND_COLOR);
        tmpBar.setUI(new BasicProgressBarUI() {
            protected Color getSelectionBackground() {
                return Color.BLACK;
            }
            protected Color getSelectionForeground() {
                return GUISetting.BACKGROUND_COLOR;
            }
        });

        tmpBar.setMaximum(GUISetting.MAX_RACE_LENGTH);
        tmpBar.setValue(GUISetting.INIT_POSITION);
        tmpBar.setString(Integer.toString(GUISetting.INIT_POSITION));
    ...
    private void createYouProgressBar(GridBagLayout gblayout, GridBagConstraints gbconstraints) {
        ...
        tmpLabel.setMinimumSize(new Dimension(GUISetting.RACER_STAT_PANEL_LABEL_WIDTH, 20));
        ...
        tmpBar.setForeground(GUISetting.LIGHT_ORANGE);
    ...
    private void createSeparatorBetweenYouAndOtherRacers(GridBagLayout gblayout, GridBagConstraints gbconstraints) {
        ...
        separator4.setBorder(BorderFactory.createMatteBorder(2, 0, 0, 0, GUISetting.BACKGROUND_COLOR));
        separator4.setAlignmentY(Component.TOP_ALIGNMENT);

        gbconstraints.gridwidth = 2; // separator will go across 2 cells
        gbconstraints.ipadx = GUISetting.RACER_STAT_PANEL_WIDTH; // padding width
    ...
    private void setRacerStatusPanelUI() {
        ...
        racerStatusPanel.setBackground(GUISetting.BACKGROUND_COLOR);
        ...
        for (int i = 0; i < GameSetting.MAX_NUM_OF_RACERS - 1; ++i) {
            JLabel tmpLabel = new JLabel();
            tmpLabel.setMinimumSize(new Dimension(GUISetting.RACER_STAT_PANEL_LABEL_WIDTH, 25));
    ...
    private void changeRacerStatusBarTheme() {
        ...
        racerStatusList.get(1).setForeground(GUISetting.LIGHT_ORANGE);
    ...
    private void setErrorPaneUI() {
        ...
        retryButton.setBackground(GUISetting.LIGHT_ORANGE);
        retryButton.setForeground(GUISetting.BACKGROUND_COLOR);
        retryButton.setBorder(new LineBorder(Color.BLACK));

        retryButton.addActionListener(e -> { Network.getInstance().connect(); });

        cancelButton = new JButton();

        cancelButton.setText("CANCEL");
        cancelButton.setPreferredSize(new Dimension(80, 25));
        cancelButton.setBackground(GUISetting.LIGHT_ORANGE);
        cancelButton.setForeground(GUISetting.BACKGROUND_COLOR);
    ...
    public void startAnswering() throws InterruptedException {
        if (GameController.getInstance().getCRacer().getStatusFlag() != GameSetting.RACER_STATUS_FLAG.FLAG_ELIMINATED) {
        ...
        CountDownLatch lock = new CountDownLatch(GameSetting.MAX_TIMER_SEC);

        timer = new Timer(1000, new ActionListener() {
            int counter = GameSetting.MAX_TIMER_SEC;
    ...
    public void setOperator(int operator) { this.operator.setText(GameSetting.OPERATORS[operator]); }
    ...
    private void submitAnswer() {
        ...
        GameController.getInstance().giveAnswer(answer);
    }
    ...
    public void resetYouProgressBar() {
        ((JProgressBar)racerStatusList.get(1)).setValue(GameSetting.INIT_RACER_POSITION); // reset progress
    ...
    public void initOpponentProgressWhenReceiveNumOfPplJoinning() {
        ((JSeparator)racerStatusList.get(2)).setBorder(BorderFactory.createMatteBorder(4, 0, 0, 0, GUISetting.BORDER_COLOR));

        for (int i = 2; i < GameController.getInstance().getNumOfRacers() + 1; ++i) {
            racerStatusList.get(i*2-1).setVisible(true); // show opponent name

            JProgressBar tmpBar = (JProgressBar)(racerStatusList.get(i*2));
            tmpBar.setForeground(GUISetting.LIGHT_ORANGE); // show opponent bar
    ...
    public void updateOpponentNameWhenJoin(Player opponent) {
        for (int i = 2; i < GameController.getInstance().getNumOfRacers() + 1; ++i) {
    ...
    public void updateOpponentProgress(Player opponent) {
        for (int i = 2; i < GameController.getInstance().getNumOfRacers() + 1; ++i) {
    ...
    public void updateOpponentProgressWhenARacerQuit(Player opponent) {
        for (int i = 2; i < GameController.getInstance().getNumOfRacers() + 1; ++i) {
            String str = ((JLabel)racerStatusList.get(i*2-1)).getText();
            ...
            if (checkIfEqualToNickname(str, opponent.getNickname())) {
                ...
                ((JProgressBar)racerStatusList.get(i*2)).setValue(GameSetting.INIT_RACER_POSITION); // reset progress
                ((JProgressBar)racerStatusList.get(i*2)).setString(Integer.toString(GameSetting.INIT_RACER_POSITION)); // reset progress number
